...
voice_asst = pyttsx3.init()
#path of the system inbuilt voice from the system registry.
# voice_id = "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech\Voices\Tokens\TTS_MS_EN-GB_HAZEL_11.0"
voices = voice_asst.getProperty('voices')
voice_asst.setProperty('voice', voices[1].id)


# define the talk function for audio messages
def talk(text):
    voice_asst.say(text)
    voice_asst.runAndWait()
...
def imgRead(img, x):
    ...
    try:
        for i in indexes.flatten():
            data_list = []
            x, y, w, h = boxs[i]
            label = str(obj_names[class_ids[i]])
            color = colors[i]
            
            cv2.rectangle(img, (x, y), (x + w, y + h), color, 2)

            if (num != id1) & (f==1):
                talk(str(obj_names[id1]) + "detected")
                data_list=[obj_names[class_ids[i]],w,(x,y)]
...
while(True):
    ...
    if out!= None:
        ...
        if not out[0]:
                ...
                if c == 1:
                    talk(str(obj_names[id1]) + "is" + str(round(distance,2)) + "feet away")
                    d=1

    if abs(int(round(time2.second - time1.second))) == 30 and d!=1:
        print('diff',abs(int(round(time2.second - time1.second))))
